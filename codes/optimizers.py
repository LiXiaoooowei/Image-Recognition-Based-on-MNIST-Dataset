...
class SGD(Optimizer):
    ...
    def update(self, xs, xs_grads, iteration):
        ...
        #############################################################
            self.moments[k] = self.moments[k] * self.momentum + xs_grads[k]
...
class Adam(Optimizer):
    ...
    def update(self, xs, xs_grads, iteration):
        ...
        #############################################################
            ...
            #first_unbias = self.moments[k] / (1 - (self.beta_1)**iteration)
            #second_unbias = self.accumulators[k] / (1 - (self.beta_2)**iteration)
            first_unbias = self.moments[k]
            second_unbias = self.accumulators[k]
            new_xs[k] = xs[k] - self.lr * first_unbias / (np.sqrt(second_unbias) + self.epsilon)
        #############################################################
        return new_xs
